...
const FinancialEducationCards = () => {
  ...
  return (
    <div className='w-max-content mx-10 '>
      <div className="flex flex-col md:flex-row gap-4 p-4 py-[100px]">
        {cards.map((card, index) => (
          <div key={index} className="rounded-xl border border-gray-200 p-6 flex flex-col items-center text-center bg-white w-full md:w-1/3">
            ...
            <h2 className="text-[24px] font-bold 2xl:text-lg mb-2">{card.title}</h2>

            <p className="text-Descride  text-descrid 2xl:text-sm mb-2">{card.subtitle}</p>
            <p className="text-Descride  text-descrid 2xl:text-sm mb-4 w-[320px]">{card.description}</p>
          </div>
        ))}
      </div>
    </div>
