...
import { useGetProfileQuery } from "../../api/auth-api"; // Fetch current user's profile
import {
  useFollowUserMutation,
  useUnfollowUserMutation,
  useGetTotalFollowersQuery,
} from "../../api/follow-api"; // Follow API hooks
...
const LessonsCard = () => {
  ...
  // Assume instructor data is passed via location.state
  const instructor = location.state?.instructor || {
    uuid: "default-uuid", // Replace with actual instructor UUID
  ...
  // Fetch current user's profile (for comparison or fallback)
  ...
  // Fetch total followers for the instructor
  const {
    data: followersData,
    isLoading: followersLoading,
  } = useGetTotalFollowersQuery(instructor.uuid, {
    skip: !instructor.uuid, // Skip if no UUID
  });
  ...
  const handleFollowToggle = async () => {
    if (!instructor.uuid) {
      alert("Instructor UUID not available.");
      return;
    }
    try {
      if (isFollowing) {
        await unfollowUser(instructor.uuid).unwrap();
        setIsFollowing(false);
      } else {
        await followUser(instructor.uuid).unwrap();
        setIsFollowing(true);
      }
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen flex">
      ...
      <aside
        className={`${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 transition-transform duration-300 fixed lg:static z-40 w-72 bg-white shadow-lg h-screen flex flex-col`}
      >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ...
                        {followersLoading ? (
                          ...
                          <span>({followersData?.total_followers || 0} followers)</span>
                        )}
                      </p>
                    </div>
                  </div>
